...
const search = (string, array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (isThere > -1) {
      ...
      if (showNet) {
        nameList.innerHTML += "Networth: $" + array[i]['networth'] + "<br>";
      }
      if (showCountry) {
        nameList.innerHTML += "Country: " + array[i]['country'] + "<br>";
      }
      if (showSource) {
        nameList.innerHTML += "Source of wealth: " + array[i]['source'] + "<br>";
      }
      if (showAge) {
        nameList.innerHTML += "Age: " + array[i]['age'] + "<br>";
      }
      if (showStatus) {
        nameList.innerHTML += "Marital status: " + array[i]['status'] + "<br>";
      }
    }
  }
}
